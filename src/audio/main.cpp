...
int main(int argc, char * argv[]){
   AudioIO * audioio = dj::audio::AudioIO::instance();
   Master * master = dj::audio::Master::instance();
   ...
   master->players()[0]->play_state(dj::audio::Player::PLAY);
   master->players()[0]->out_state(dj::audio::Player::MAIN_MIX);
...
int main(int argc, char * argv[]){
   if(argc < 3)
      return -1;
   dj::audio::AudioIO * audioio = dj::audio::AudioIO::instance();
   cout << "reading in buffers" << endl;
   //dj::audio::AudioBuffer buffer2("/mp3/model_500/classics/02-the_chase_smooth_remix.flac");
   //dj::audio::BeatBuffer beatBuffer2("/home/alex/.datajockey/annotation/2.yaml");
   //dj::audio::AudioBuffer buffer("/mp3/new_order/movement/01-dreams_never_end.flac");
   //dj::audio::BeatBuffer beatBuffer("/home/alex/.datajockey/annotation/2186.yaml");
   dj::audio::AudioBuffer buffer(argv[1]);
   //dj::audio::BeatBuffer beatBuffer("/home/alex/music/annotation/377.yaml");
   dj::audio::AudioBuffer buffer2(argv[2]);
   //dj::audio::BeatBuffer beatBuffer2("/home/alex/music/annotation/441.yaml");
   dj::audio::Master * master = dj::audio::Master::instance();
   master->add_player();
   master->add_player();
   ...
   master->players()[1]->play_state(dj::audio::Player::PLAY);
   master->players()[1]->play_speed(1.5);
   master->players()[1]->out_state(dj::audio::Player::MAIN_MIX);
...

   {
      ...
      master->scheduler()->remove(
            master->scheduler()->schedule( pos,
               new dj::audio::PlayerStateCommand(1,
                  dj::audio::PlayerStateCommand::NO_LOOP)
               ));
...

   {
      ...
      for(i = 0; i < 16; i++){
         pos.at_bar(i / 4, i % 4);
         master->scheduler()->schedule( pos,
               new dj::audio::PlayerStateCommand(i % 2, 
                  dj::audio::PlayerStateCommand::MUTE)
               );
         master->scheduler()->schedule( pos,
               new dj::audio::PlayerStateCommand((i + 1) % 2, 
                  dj::audio::PlayerStateCommand::NO_MUTE)
               );
      }
      pos.at_bar(i / 4, i % 4);
      master->scheduler()->schedule( pos,
...

   {
      ...
      master->scheduler()->execute(
            new dj::audio::PlayerPositionCommand(0,
               dj::audio::PlayerPositionCommand::PLAY,
               pos)
            );
      ...
      pos.at_bar(1, 1);
      master->scheduler()->execute(
            new dj::audio::PlayerPositionCommand(1,
               dj::audio::PlayerPositionCommand::LOOP_START,
               pos)
            );
      master->scheduler()->execute(
            new dj::audio::PlayerPositionCommand(1,
      ...
      master->scheduler()->execute(
            new dj::audio::PlayerPositionCommand(1,
               dj::audio::PlayerPositionCommand::LOOP_END,
               pos)
            );
      master->scheduler()->execute(
            new dj::audio::PlayerStateCommand(1,
               dj::audio::PlayerStateCommand::LOOP)
            );
   }
   ...
   sleep(1);
   {
      ...
      master->scheduler()->execute(
            new dj::audio::PlayerStateCommand(1,
               dj::audio::PlayerStateCommand::NO_SYNC)
            );
   }
